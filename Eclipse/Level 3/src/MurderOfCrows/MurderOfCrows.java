...
public class MurderOfCrows {
	...
	private void findTheDiamond() {
		 ...
		 */
		int crowCounter = 0;
		String response = " Crows were killed.";
		for (Crow c : theMurder) {
			for (String s : c.getStomachContents()) {
				if (s.equals("diamond")) {
					System.out.println(c.getName() + " had the diamond.");
					if (crowCounter == 1) {
						response = " Crow was killed.";
					}
					System.out.println(crowCounter + response);
				}
			}
			crowCounter++;
		}
